...
export default function HomeScreen() {
  ...
  async function handleSignIn() {
    setSigningIn(true)
    try { await signInWithGoogle() } catch (e) { if (e?.code !== 'auth/popup-closed-by-user') console.error(e) }
    setSigningIn(false)
  }
      ...
      {user !== undefined && (
        user
          ...
          : <button
              ...
              disabled={signingIn}
              className="flex items-center justify-center gap-2 text-gray-400 text-sm py-2 disabled:opacity-60"
            >
              <GoogleIcon />
